...
def evaluate(model, test_loader, PAD_INDEX, UNK_INDEX):
    ...
    wandb.log({'AUC-ROC' : wandb.plot.roc_curve(target.cpu() ,torch.argmax(output.cpu(), axis=-1), labels=[0, 1])})
    wandb.log({'Precision_recall' : wandb.plot.pr_curve(target.cpu(), torch.argmax(output.cpu(), axis=-1), labels=[0, 1])})
